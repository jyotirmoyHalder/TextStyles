...
//        myString.baselineOffset = 30
        myString.kern = 5
        return myString
    }
    
    var attr2Demo: AttributedString {
        let codingIsAwesome = "Coding is Awesome! Let's coe together!"
...
//    let listOfWord = [ "bread", "salt", "domino", "radius", "child", "foot", "tooth", "man", "woman", "goose", "focus", "matrix", "deer", "oasis", "fish", "index", "boom", "brush"]
    
    var attr4Demo: AttributedString {
        let codingIsAwesome = "Coding is Awesome! Let's code together!"
        var myString = AttributedString()
        let listOfWord = codingIsAwesome.split(separator: " ")
        myString.font = Font.title
        
        for (i, word) in listOfWord.enumerated() {
